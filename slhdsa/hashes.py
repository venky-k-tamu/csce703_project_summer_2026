...
All hash functions are instantiated with SHAKE256, regardless of
security category — the "128" in SLH-DSA-SHAKE-128s names the
parameter set's classical security level (driven by n, h, d, k, ...),
not the XOF variant. FIPS 205 §10.2 fixes SHAKE256 for every
...
def F(pk_seed: bytes, adrs: bytearray, m1: bytes) -> bytes:
    ...
    return _shake256(bytes(pk_seed) + bytes(adrs) + bytes(m1), N)


def H(pk_seed: bytes, adrs: bytearray, m: bytes) -> bytes:
    """2-input T-hash (§10.2). Inputs: PK.seed (n), ADRS (32), M (2n)."""
    return _shake256(bytes(pk_seed) + bytes(adrs) + bytes(m), N)
